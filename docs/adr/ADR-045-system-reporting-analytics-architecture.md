...
**Discharges:** TASK-ECOS-SYSTEM-REPORTING-ARCHITECTURE-001 (Task 1 of System Reporting Batch 01), remediated by TASK-ECOS-SYSTEM-REPORTING-ARCHITECTURE-001-R1 (ADR renumbered 044→045; Finance source-state wording corrected — see §0 and "Known Documentation Gaps")
...
5. **Financial and operational truth are not the same number in this workspace's local integrated baseline (`16b0ec85`)**: only `pos.sale.finalized` posts to the General Ledger; the delivery/COD sales channel — ECOS's primary channel — posts nothing at that baseline (`Finance\Integration\Application\Bridge\EventPostingCatalog` has no `orders.delivered`/`inventory.stock.shipped` entry), a gap Finance's own architecture-lock audit and UAT-009 report both certify as real at that snapshot. **A remediation pass (TASK-...-001-R1) subsequently verified, by direct read-only source inspection of the sibling `ecos-finance` lane (branch `task/finance-gap-closure`, HEAD `950a2817`), that an unintegrated Finance candidate workstream ("Task 6 — Commercial Accounting") has since implemented the missing wiring** — a genuine, traced, working call chain from `OrderDeliveredEvent`/`CodCollected` through to real `JournalEngine::post()` calls for delivery revenue+AR+tax, delivery COGS, and COD cash settlement (full citations: engineering report §10, §18, §33). That candidate work is itself explicitly untested and unintegrated by its own authors' record (`TESTS EXECUTED: NO`, `CERTIFIED: NO`, `INTEGRATED: NO`), and its relationship to the true first-device canonical `develop` is unverified from this device. See the Source-State Model immediately below — every finding in this ADR about Finance revenue/COGS is now stated against one of its three named states, never as an unqualified system-wide absolute. A Reporting layer that ignored any of this and quietly equated "order value" with "GL revenue" — in either direction, either by assuming the baseline gap is permanent or by assuming the candidate is already canonical — would manufacture a false number, not just a stylistic inconsistency.

### Source-State Model (locked by this ADR)

Every factual claim in this ADR and its accompanying documents about what exists "in Finance," "in Commerce," or in any other module is stated against exactly one of three named states — never left ambiguous about which:

| State | Meaning | As observed in this task |
|---|---|---|
| **A. Local Integrated Baseline** | This workspace's own committed code, at the HEAD this task ran against | `16b0ec85df5774f03ccd6dca042528260d66c216` |
| **B. Unintegrated Source Candidate** | Real code in a sibling lane's own branch, not merged to shared `develop`, verified by direct read-only inspection but not assumed integrated | e.g. `ecos-finance` @ `task/finance-gap-closure`, `ecos-iam` @ `task/iam-workstream`, `ecos-chat` @ `task/chat-workstream` |
| **C. First-Device Canonical Develop** | The true, authoritative integrated source | `E:\ECOS\ecos-develop` — **NOT VERIFIED** from this device (§0) |

**Rule:** never describe B as integrated into A or C. Never describe A as if it were C. A confirmed gap in A is reported as a baseline finding, not a permanent system-wide fact, once a State-B candidate addressing it has been directly verified to exist.
...
**Decision 2b (scope of ADR-020).** ADR-020's claim that "the snapshot IS the financial truth" is scoped to **Commerce order-economics** (`order_financial_snapshots`/`order_line_snapshots` — grand total, COGS-at-sale, margin, captured once at order-Confirm). It is not a statement about the General Ledger. In the Local Integrated Baseline (State A, `16b0ec85`) it is not wired to the GL at all; an Unintegrated Finance Candidate (State B, `ecos-finance` Task 6) has since built that wiring, verified working but not canonical (see Source-State Model, Context §5). Every Metric Dictionary entry sourced from Commerce's snapshot tables must carry the `OPERATIONAL` classification, never `ACCOUNTING`, and must never be relabeled "Recognized Revenue" — this rule holds regardless of which source-state Finance's own GL-side wiring is in, because Commerce's snapshot was never intended to be the GL figure in the first place.
...
Per task §42/§15, Reporting may call Finance's queries and display Finance's aggregates; it may not implement its own P&L, AR/AP balance, revenue recognition, COGS, or Driver financial balance — **this rule is unaffected by which source-state Finance's own wiring happens to be in**. Concretely: nearly every Financial report classifies as **READY IN LOCAL BASELINE** (Trial Balance, Income Statement, Balance Sheet, AR/AP Aging, Cash Position, Profitability, Closing reports are all live and routed in State A today). The delivery/COD revenue-recognition gap (Decision 2b) is a **State-A baseline finding** with a verified **State-B candidate fix** (`ecos-finance` Task 6 — Revenue/AR, COGS, and COD-settlement postings all confirmed FULLY IMPLEMENTED AND WIRED by direct source inspection, engineering report §10/§18/§33) whose canonical (State C) integration is **NOT YET VERIFIED FROM THIS DEVICE**. Reporting must **not** attempt to close this gap itself in any state — neither by inventing its own number against the State-A gap, nor by prematurely treating the State-B candidate as canonical before it is confirmed integrated. The one genuinely missing piece regardless of state is a **frontend surface** for Customer/Supplier Statements — `CustomerLedgerService::statement()` / `SupplierLedgerService::statement()` are implemented and routed in State A today but never called from any frontend page. Reporting's Financial category in V1 is therefore overwhelmingly "expose and govern," not "build."
...
4. **ADR-044 collision — found and resolved by TASK-...-001-R1.** This document was originally filed as ADR-044. A CTO review confirmed the sibling `ecos-chat` lane (branch `task/chat-workstream`, local HEAD `12f8ab55`) had already committed `ADR-044-internal-collaboration-bounded-context.md` to its own candidate history first. A remediation pass re-checked every locally-visible ADR index (`ecos-reporting`, `ecos-finance`, `ecos-iam`, `ecos-chat`) and confirmed no `ADR-045` or higher existed anywhere before renumbering this document to **ADR-045**. `ecos-chat` retains ADR-044 for internal-collaboration-bounded-context. This resolves the collision **locally** only — first-device canonical `develop` (State C) may contain other ADR-044/045 claims not visible from this device; re-verify at the first opportunity to reach `E:\ECOS\ecos-develop` (Task 5).
