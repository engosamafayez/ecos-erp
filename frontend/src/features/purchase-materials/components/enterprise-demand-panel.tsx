  ...
  return (
    <section>
      ...
      <div className="rounded-md border bg-background divide-y">
        <StatRow label="Warehouses Carrying" value={bi.warehouses_carrying} highlight />
  ...
  return (
    <section>
      ...
      <div className="rounded-md border bg-background divide-y">
        ...
        <StatRow label="In Transfer" value={health.in_transfer > 0 ? fmt(health.in_transfer, 0) : '—'} />
    ...
    <section>
      <div className="flex items-center justify-between mb-2">
        ...
        <div className="flex items-center gap-1 text-[10px] text-muted-foreground -mt-1">
          ...
          <span className="capitalize">{demand.trend === 'normal' ? 'Normal trend' : demand.trend + ' than avg'}</span>
        </div>
      </div>
      <div className="grid grid-cols-3 gap-1.5 mb-2">
        {[
      ...
      <div className="rounded-md border bg-background divide-y">
        <StatRow label="90-Day Rolling Avg" value={fmt(demand.rolling_90d_avg, 2)} />
  ...
  return (
    <section>
      ...
      <div className="rounded-md border bg-background divide-y mb-2">
        ...
        <StatRow label="Average Cost" value={proc.avg_cost != null ? fmt(proc.avg_cost, 2) : '—'} />
        ...
        <StatRow label="Purchase Frequency" value={proc.purchase_frequency != null ? `${fmt(proc.purchase_frequency, 1)}×/mo` : '—'} />
              ...
              <div key={s.supplier_id} className="rounded-md border bg-background px-3 py-2 flex items-center justify-between">
                <div>
                  ...
                  {s.last_delivery_date && (
                    <p className="text-muted-foreground text-[10px]">Last: {fmtDate(s.last_delivery_date)}</p>
                  )}
                </div>
                <div className="text-right">
                  ...
                  {s.lead_time_days != null && (
                    <p className="text-muted-foreground text-[10px]">{s.lead_time_days}d lead</p>
                  )}
                </div>
              </div>
            ))}
    ...
    return (
      <section>
        ...
        <div className="flex items-center gap-2 text-xs text-emerald-700 rounded-md border border-emerald-200 bg-emerald-50 px-3 py-2">
          ...
          No issues detected. Inventory looks good.
        </div>
      </section>
    );
  }
  ...
  if (!productId) {
    return (
      <div className="flex flex-col items-center justify-center h-full gap-2 text-sm text-muted-foreground text-center px-4">
        ...
        <p>Select a material to see demand intelligence and procurement recommendations.</p>
      </div>
    );
  }
