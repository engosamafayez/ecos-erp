  ...
  return (
    <div className="flex flex-col h-full">
      <PageHeader
        ...
        subtitle="Manage supply requests, supplier selection, and purchasing decisions."
        actions={
      ...
      <div className="flex-1 overflow-auto px-6 pb-6 flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-3">
          {/* Operational group */}
          <div>
            <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">OPERATIONS</p>
        ...
        <div className="flex flex-col gap-2 rounded-lg border bg-muted/20 p-3">
          ...
          <div className="flex flex-wrap gap-2 items-center">
            <Input
              ...
              placeholder="Search by request no. or notes…"
              value={search}
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm whitespace-nowrap">
                <thead className="bg-muted/40 border-b">
                  <tr>
                    <th className="px-3 py-3 text-start font-medium text-xs text-muted-foreground">Request No.</th>
                    <th className="px-3 py-3 text-start font-medium text-xs text-muted-foreground">Company</th>
                    <th className="px-3 py-3 text-start font-medium text-xs text-muted-foreground">Channel</th>
                    <th className="px-3 py-3 text-start font-medium text-xs text-muted-foreground">Warehouse</th>
                    <th className="px-3 py-3 text-start font-medium text-xs text-muted-foreground">Requested By</th>
                    <th className="px-3 py-3 text-center font-medium text-xs text-muted-foreground">Items</th>
                    <th className="px-3 py-3 text-end font-medium text-xs text-muted-foreground">Requested Qty</th>
                    ...
                    <th className="px-3 py-3 text-start font-medium text-xs text-muted-foreground">Assigned Buyer</th>
                ...
                <tbody>
                  {isLoading ? (
                    ...
                    <tr>
                      <td colSpan={14} className="px-4 py-12 text-center text-sm text-muted-foreground">
                        {search || statusFilter !== 'all' ? 'No requests match the current filters.' : 'No material requests yet.'}
                      </td>
                    </tr>
                  ) : (
                    items.map((pm) => (
