...
import { useIntelligenceAds, buildExportUrl } from '../../hooks/use-intelligence';
...
import type { IntelligenceFilters, AdAnalyticsRow } from '../../types/intelligence';
...
const COLUMNS: DataGridColumnDef<AdAnalyticsRow>[] = [
  {
    key:   'name',
    label: 'Ad Name',
    alwaysVisible: true,
...
export function AdAnalyticsPage() {
  ...
  const { data, isLoading, isError, isFetching, refetch } = useIntelligenceAds(filters);
  ...
  function triggerExport(format: 'csv' | 'excel' | 'html') {
    window.open(buildExportUrl('ads', filters, format), '_blank');
  }
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="border-b bg-background px-6 py-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-lg font-semibold">Ad Analytics</h1>
          {meta && (
            <p className="text-xs text-muted-foreground mt-0.5">
              {meta.total.toLocaleString()} ads · {meta.date_from} – {meta.date_to}
            </p>
          )}
        </div>
