...
function BusinessContextSection({ ctx }: { ctx: OrderBusinessContextSnapshot }) {
  return (
    <div className="flex flex-col gap-4">
      ...
      {(ctx.brand_context.name || ctx.channel_context.name) && (
        <div className="rounded-lg border p-3 space-y-0.5">
          {ctx.brand_context.name && (
            <Row label="Brand" value={ctx.brand_context.name} />
          )}
          {ctx.channel_context.name && (
            <Row label="Channel" value={ctx.channel_context.name} />
          )}
          {ctx.channel_context.type && (
            <Row label="Channel type" value={ctx.channel_context.type} />
          )}
        </div>
      )}
      ...
      <div className="rounded-lg border p-3 space-y-0.5">
        ...
        {ctx.decision_provenance.price.source && (
          <Row label="Price source" value={ctx.decision_provenance.price.source} />
        )}
        {ctx.decision_provenance.cost.source && (
          <Row label="Cost source" value={ctx.decision_provenance.cost.source} />
        )}
        {ctx.decision_provenance.cost.recipe_version && (
          <Row label="Recipe version" value={`v${ctx.decision_provenance.cost.recipe_version}`} />
        )}
        {ctx.decision_provenance.discount.source && (
          <Row
            label="Discount source"
            value={ctx.decision_provenance.discount.manual_override ? 'Manual override' : ctx.decision_provenance.discount.source}
            highlight={ctx.decision_provenance.discount.manual_override ? 'amber' : undefined}
          />
        )}
        {ctx.decision_provenance.shipping.zone && (
          <Row label="Shipping zone" value={ctx.decision_provenance.shipping.zone} />
        )}
      </div>
      ...
      {ctx.customer_context.delivery_success_rate != null && (
        <div className="rounded-lg border p-3 space-y-0.5">
          ...
          {ctx.customer_context.tier && (
            <Row label="Customer tier" value={ctx.customer_context.tier} />
          )}
          {ctx.customer_context.segment && (
            <Row label="Segment" value={ctx.customer_context.segment} />
          )}
        </div>
      )}
      ...
      {(ctx.policy_versions.pricing || ctx.policy_versions.shipping) && (
        <div className="rounded-lg border p-3 space-y-0.5">
          ...
          {ctx.policy_versions.pricing && (
            <Row label="Pricing policy" value={`v${ctx.policy_versions.pricing}`} />
          )}
          {ctx.policy_versions.shipping && (
            <Row label="Shipping policy" value={`v${ctx.policy_versions.shipping}`} />
          )}
        </div>
      )}
      ...
      {(ctx.marketing_context.campaign_name || ctx.marketing_context.utm_source) && (
        <div className="rounded-lg border p-3 space-y-0.5">
          ...
          {ctx.marketing_context.campaign_name && (
            <Row label="Campaign" value={ctx.marketing_context.campaign_name} />
          )}
          {ctx.marketing_context.utm_source && (
            <Row label="UTM source" value={ctx.marketing_context.utm_source} />
          )}
          {ctx.marketing_context.utm_medium && (
            <Row label="UTM medium" value={ctx.marketing_context.utm_medium} />
          )}
        </div>
      )}
    </div>
  ...
  return (
    <div className="rounded-lg border bg-muted/20 p-3 text-sm">
      ...
      <div className="border-t pt-2 mt-2 space-y-0.5">
        <Row label="Price at sale" value={fmt(line.unit_price_at_sale)} mono />
        {line.regular_price_at_sale != null && (
          <Row label="Regular price" value={fmt(line.regular_price_at_sale)} mono />
        )}
        {line.sale_price_at_sale != null && (
          <Row label="Sale price" value={fmt(line.sale_price_at_sale)} mono />
        )}
      </div>
      ...
      {hasCost && (
        <div className="border-t pt-2 mt-2 space-y-0.5">
          {line.raw_material_cost != null && (
            <Row label="Raw materials" value={fmt(line.raw_material_cost)} mono />
          )}
          {line.packaging_cost != null && line.packaging_cost > 0 && (
            <Row label="Packaging" value={fmt(line.packaging_cost)} mono />
          )}
          {line.manufacturing_cost != null && line.manufacturing_cost > 0 && (
            <Row label="Manufacturing" value={fmt(line.manufacturing_cost)} mono />
          )}
          {line.other_cost != null && line.other_cost > 0 && (
            <Row label="Other" value={fmt(line.other_cost)} mono />
          )}
          <Row label="Unit cost" value={fmt(line.unit_cost)} mono />
          <Row label="Line cost" value={fmt(line.line_cost)} mono />
        </div>
      )}
      ...
      {line.gross_profit != null && (
        <div className="border-t pt-2 mt-2 space-y-0.5">
          ...
          {line.target_margin_percent != null && (
            <Row label="Target margin" value={fmtPct(line.target_margin_percent)} />
          )}
          {line.source_recipe_version && (
            <p className="text-[10px] text-muted-foreground mt-1">
              Recipe v{line.source_recipe_version}
              {line.bom_version_number != null ? ` · BOM #${line.bom_version_number}` : ''}
            </p>
          )}
          {line.price_review_id && (
            <p className="text-[10px] text-muted-foreground">
              Price review approved{line.price_review_approved_at
...
export function OrderSnapshotTab({ orderId }: OrderSnapshotTabProps) {
  ...
  if (!snapshot) {
    return (
      <div className="flex flex-col items-center gap-3 py-16 text-center px-4">
        ...
        <p className="text-sm font-medium">Snapshot Not Available</p>
        <p className="text-xs text-muted-foreground max-w-xs">
          The financial snapshot is created when the order is first confirmed.
          Move this order to a confirmed status to lock its financials.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-6 p-4">
      ...
      {/* ── PART 14: Version badge + Locked indicator ── */}
      ...
      {/* ── PART 5+6: SHA-256 integrity health indicator ── */}
      {snapshot.hash_verified === false ? (
        <div className="flex items-center gap-2 rounded-md border border-destructive/40 bg-destructive/5 px-3 py-2">
          ...
          <p className="text-xs text-destructive font-medium">
            Integrity check failed — SHA-256 mismatch. Contact your system administrator.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 rounded-md border border-emerald-200 bg-emerald-50 px-3 py-2 dark:border-emerald-900/50 dark:bg-emerald-950/30">
          ...
          <p className="text-xs text-emerald-700 dark:text-emerald-400">
            Snapshot verified — integrity hash matches stored data.
          </p>
        </div>
      ) : (
        <div className="flex items-center gap-2 rounded-md border border-amber-200 bg-amber-50 px-3 py-2 dark:border-amber-900/50 dark:bg-amber-950/30">
          ...
          <p className="text-xs text-amber-700 dark:text-amber-400">
            This snapshot is immutable. It reflects the exact financial truth at confirmation time.
          </p>
        </div>
      )}
      ...
      {/* ── PART 9: Snapshot Timeline ── */}
      <div>
        <SectionTitle>Snapshot Timeline</SectionTitle>
        <SnapshotTimeline
          events={[
            {
            ...
            {
              label: 'Financial Snapshot Created',
      ...
      {snapshot.business_context && (
        <div>
          ...
          <BusinessContextSection ctx={snapshot.business_context} />
        </div>
      )}
      ...
      <div>
        ...
        <div className="rounded-lg border p-3">
          <Row label="Subtotal" value={fmt(snapshot.subtotal)} mono />
          {snapshot.discount_amount > 0 && (
            <Row
              label={`Discount${snapshot.discount_type === 'percentage' ? ' (%)' : ''}`}
          ...
          {snapshot.shipping_cost > 0 && (
            <Row label="Shipping" value={fmt(snapshot.shipping_cost)} mono />
          )}
          <Separator className="my-1.5" />
          <Row label="Grand total" value={fmt(snapshot.grand_total)} mono />
          {snapshot.deposit_amount > 0 && (
            <>
              <Row label="Deposit paid"      value={`−${fmt(snapshot.deposit_amount)}`} mono highlight="green" />
              <Row label="Remaining balance" value={fmt(snapshot.remaining_balance)}    mono />
            </>
          )}
        </div>
      </div>
        ...
        <div>
          ...
          <div className="rounded-lg border p-3">
            {snapshot.shipping_zone && (
              <Row label="Zone" value={snapshot.shipping_zone} />
            )}
            {snapshot.shipping_rule_name && (
              <Row label="Rule" value={snapshot.shipping_rule_name} />
            )}
            <Row
              label="Override"
              value={snapshot.shipping_override_applied ? 'Yes — manual override' : 'No — auto pricing'}
      ...
      {hasCostData && (
        <div>
          ...
          <div className="rounded-lg border p-3">
            <Row label="Total COGS"      value={fmt(snapshot.total_cogs)} mono />
            {snapshot.total_raw_material_cost != null && snapshot.total_raw_material_cost > 0 && (
              <Row label="  Raw materials"  value={fmt(snapshot.total_raw_material_cost)} mono />
            )}
            {snapshot.total_packaging_cost != null && snapshot.total_packaging_cost > 0 && (
              <Row label="  Packaging"      value={fmt(snapshot.total_packaging_cost)} mono />
            )}
            {snapshot.total_manufacturing_cost != null && snapshot.total_manufacturing_cost > 0 && (
              <Row label="  Manufacturing"  value={fmt(snapshot.total_manufacturing_cost)} mono />
            )}
            {snapshot.total_other_cost != null && snapshot.total_other_cost > 0 && (
              <Row label="  Other"          value={fmt(snapshot.total_other_cost)} mono />
            )}
            <Separator className="my-1.5" />
            <Row
              label="Gross profit"
            ...
            {snapshot.target_margin_percent != null && (
              <Row label="Target margin" value={fmtPct(snapshot.target_margin_percent)} />
            )}
        ...
        <div>
          <SectionTitle>
            <span className="inline-flex items-center gap-1.5">
              ...
              Line Detail ({snapshot.lines.length})
            </span>
          </SectionTitle>
          <div className="flex flex-col gap-2">
            {snapshot.lines.map((line) => (
              <LineSnapshotRow key={line.id} line={line} />
            ))}
          </div>
        </div>
      )}
      ...
      {/* ── Snapshot metadata ── */}
      <div>
        <SectionTitle>Snapshot Metadata</SectionTitle>
