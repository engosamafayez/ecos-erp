      ...
      <div className="flex flex-col gap-3">
        ...
        {singleResult ? (
          <CustomerQuickActionCard
            ...
            onEdit={openEdit}
            // Was a literal no-op (`() => undefined`) — a rendered, tappable
            // "Create Order" button that did nothing on tap. Fixed to the real
            // canonical order-creation route (TASK-ECOS-MOBILE-UX-COMPLETION-
            // 003 §13: no placeholder callbacks). Does not pre-fill the
            // customer — the manual order form's own phone lookup drives
            // customer selection; wiring a pre-fill would be new Orders-
            // feature plumbing, out of this task's scope.
            onCreateOrder={() => navigate(ROUTES.ordersNew)}
            onClose={() => setSearch('')}
            className="max-w-md"
          />
        ) : null}
