...
function Timeline({ session }: { session: CountSession }) {
  const steps: { label: string; ts: string | null; done: boolean }[] = [
    ...
    { label: 'Count Finished',     ts: session.completed_at, done: !!session.completed_at },
    ...
    <div className="flex-1 overflow-auto p-6 space-y-6">
      ...
      <div className="rounded-lg border bg-card p-4">
        ...
        <div className="grid grid-cols-2 gap-x-6 gap-y-1 text-xs">
          ...
          <div className="text-muted-foreground">Approved At</div><div>{fmtDateTime(report.session.approved_at)}</div>
        </div>
      </div>
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b bg-muted/40">
                ...
                <th className="px-3 py-2 text-right font-medium text-muted-foreground">System</th>
            ...
            <tbody>
              {report.product_details.map((row) => (
                <tr key={row.id} className="border-b last:border-0 hover:bg-muted/30">
                  ...
                  <td className="px-3 py-2 text-right tabular-nums">{fmt(row.system_qty, 0)}</td>
                  <td className="px-3 py-2 text-right tabular-nums">{fmt(row.counted_qty, 0)}</td>
                  <td className={`px-3 py-2 text-right tabular-nums ${row.damaged_qty > 0 ? 'text-amber-600 font-medium' : 'text-muted-foreground'}`}>
                    {row.damaged_qty > 0 ? fmt(row.damaged_qty, 0) : '—'}
                  </td>
                  <td className={`px-3 py-2 text-right tabular-nums ${(row.shortage_qty ?? 0) > 0 ? 'text-destructive font-medium' : 'text-muted-foreground'}`}>
...
export function CountSessionDrawer({ sessionId, open, onOpenChange }: Props) {
  ...
  async function handleAction(action: () => Promise<unknown>, label: string) {
    try {
      ...
      toast.error(`Failed to ${label} session.`);
    }
  }
      ...
      <SheetContent className="w-full sm:max-w-4xl flex flex-col p-0 gap-0 overflow-hidden">
        {isLoading ? (
          ...
          <SessionContent
            ...
            onStart={() => handleAction(() => startMutation.mutateAsync(session.id), 'started')}
            onComplete={() => handleAction(() => completeMutation.mutateAsync(session.id), 'completed')}
            onApprove={() => handleAction(() => approveMutation.mutateAsync({ id: session.id }), 'approved')}
            onCancel={() => handleAction(() => cancelMutation.mutateAsync(session.id), 'cancelled')}
          />
        )}
      </SheetContent>
    </Sheet>
      ...
      {(session.status === 'in_progress' || session.status === 'completed') && (
        <Alert className="mx-6 mt-3 shrink-0 border-amber-200 bg-amber-50 dark:bg-amber-950/20">
          <AlertDescription className="text-xs text-amber-800 dark:text-amber-200">
            <strong>Blind Count Active</strong> — System quantities, variance, and financial impact are hidden until the session is approved, preventing bias during the physical count.
          </AlertDescription>
        </Alert>
      )}
      ...
      {view === 'report' && session.status === 'approved' ? (
        ...
        <>
          ...
          {session.status === 'approved' && (totalShortageLines > 0 || totalDamagedLines > 0) && (
            <Alert className="mx-6 my-3 shrink-0">
              <AlertDescription className="text-xs">
                {totalShortageLines > 0 && `${totalShortageLines} shortage(s) created as Warehouse Liabilities. `}
                {totalDamagedLines > 0 && `${totalDamagedLines} damaged item(s) require Waste Investigation.`}
              </AlertDescription>
            </Alert>
          )}
          ...
          <div className="flex-1 overflow-auto">
            {lines.length === 0 ? (
              <p className="text-muted-foreground text-sm text-center py-12">No items in this count session.</p>
            ) : (
              <table className="w-full text-sm">
                <thead className="sticky top-0 bg-background z-10 shadow-sm">
                  <tr className="border-b">
                    ...
                    {showSystemQty && (
                      <th className="px-3 py-2 text-right text-xs font-medium text-muted-foreground">System Qty</th>
                    )}
                    <th className="px-3 py-2 text-left text-xs font-medium text-muted-foreground">Counted Qty</th>
                    <th className="px-3 py-2 text-left text-xs font-medium text-muted-foreground">Damaged Qty</th>
                    <th className="px-3 py-2 text-left text-xs font-medium text-muted-foreground">Damage Reason</th>
                    {showSystemQty && (
                      <th className="px-3 py-2 text-right text-xs font-medium text-muted-foreground">Shortage</th>
                    )}
                    {showSystemQty && (
                      <th className="px-3 py-2 text-right text-xs font-medium text-muted-foreground">Variance</th>
                    )}
                    <th className="px-3 py-2 text-left text-xs font-medium text-muted-foreground">Attachments</th>
                  </tr>
                </thead>
                <tbody>
                  {lines.map((line) => (
