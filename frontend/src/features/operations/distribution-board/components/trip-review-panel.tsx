  ...
  return (
    <div className="space-y-6">
      ...
      <section>
        ...
        {!manifest ? (
          <p className="text-sm text-muted-foreground">No manifest generated.</p>
        ) : manifest.items.length === 0 ? (
          <p className="text-sm text-muted-foreground">No products in manifest.</p>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b bg-muted/40">
                  ...
                  <th className="px-3 py-2 text-right font-medium text-muted-foreground text-xs">Required</th>
                  <th className="px-3 py-2 text-right font-medium text-muted-foreground text-xs">Loaded</th>
                  <th className="px-3 py-2 text-right font-medium text-muted-foreground text-xs">Shortage</th>
                  <th className="px-3 py-2 text-right font-medium text-muted-foreground text-xs">Status</th>
                </tr>
              </thead>
              <tbody className="divide-y">
                {manifest.items.map((item) => (
                  <tr key={item.id} className="hover:bg-muted/20">
                    ...
                    <td className="px-3 py-2 text-right tabular-nums">{item.required_qty}</td>
      ...
      {custody.total === 0 && (
        <section>
          <h3 className="text-sm font-semibold mb-2">Custody Items</h3>
