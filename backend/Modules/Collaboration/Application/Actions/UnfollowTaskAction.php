...
final class UnfollowTaskAction extends BaseAction
{
    ...
    public function execute(mixed ...$arguments): InternalTask
    {
        ...
        if (! $actor instanceof User || ! $task instanceof InternalTask || ! $target instanceof User) {
            throw new InvalidArgumentException('UnfollowTaskAction::execute expects (User $actor, InternalTask $task, User $target).');
        }

        if (! $this->policy->manageFollower($actor, $task, $target)) {
            throw new AuthorizationException('You do not have access to remove this follower.');
        }

        $task->followers()->where('user_id', $target->id)->delete();
