<?php

declare(strict_types=1);

namespace Modules\Commerce\Synchronization\Application\Observers;

use Modules\Commerce\Orders\Domain\Models\Order;
use Modules\Commerce\Synchronization\Application\Jobs\OrderStatusSyncJob;

final class OrderObserver
{
    public function updated(Order $order): void
    {
        if (! $order->wasChanged('status')) {
            return;
        }

        if ($order->external_order_id === null || $order->external_order_id === '') {
            return;
        }

        if ($order->channel_id === null) {
            return;
        }

        $channel = $order->channel;

        // TASK-...-025 (W6) — Orders Sync pause gate. `is_active` is the channel-wide kill
        // switch; `sync_orders` is the Orders-specific pause this task adds. Both must hold.
        // TASK-...-WOO-04 — a channel not yet Live stays inert regardless of either flag
        // (042A-R1 §5). TASK-...-CONSOLIDATED-REMEDIATION-001-R2-R1 §14 — canSyncNow() also
        // closes the gate for a paired Channel whose Connector has gone stale/disconnected.
        if ($channel === null || ! $channel->is_active || ! $channel->sync_orders || ! $channel->canSyncNow()) {
            return;
        }

        OrderStatusSyncJob::dispatch($channel, $order);
    }
}
